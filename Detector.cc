...
Detector::Detector(Settings * settings1, IceModel * icesurface, string setupfile) {
    ...
    /////////////////////////////////////////////////////////////////////////////////    
        ...
        for (int i = 0; i < stations.size(); i++) {
            ...
            if (settings1 -> USE_INSTALLED_TRIGGER_SETTINGS == 0) {
                ...
                else if (stations[i].StationID == 1) {
                    stations[i].NFOUR = 1024;
                    stations[i].TIMESTEP = 1. / 2.6 * 1.E-9;
                    stations[i].TRIG_WINDOW = 2.5E-7;
                    stations[i].DATA_BIN_SIZE = settings1 -> DATA_BIN_SIZE;
    ...
    /////////////////////////////////////////////////////////////////////////////
        ...
        if(settings1->NOISE==1){
            ...
            char the_rayleigh_filename[500];
            sprintf(the_rayleigh_filename, "%s/data/noise/sigmavsfreq_PA_config_%d.csv", getenv("ARA_SIM_DIR"),
        ...
        if(words.size() > 0 && words[0] == "n") { // index included in file; when included expect format like "n : 1.0"
            ...
            if(freq_step == -1 || type == eTx) {
                *source_n = buff_n;
                ...
                if(buff_n != *source_n) {
                    throw runtime_error("Antenna gain file's index of refraction does not match those already loaded! "+filename);
                }
            }
...
double Detector::GetGain_1D_OutZero( double freq, double theta, double phi, int ant_m, double antenna_target_medium_n, int string_number, int ant_number, bool useInTransmitterMode) {
    ...
    */
    ...
    // scale frequency according to ratio of media indices of refraction
    // if source medium n is a valid value scale the frequency, otherwise do not
    double freq_scaled = (source_n >= 1)? freq * antenna_target_medium_n / source_n : freq; 
...
inline void Detector::ReadCalPulserWF(string filename, Settings *settings1 ) {    // will store calpulser waveform array
    ...
    if ( CalPulWF.is_open() ) {
        ...
        cout<<"CalPulserWF file can not opened!!"<<endl;
    ...
    if(gainFile.is_open()){
        while(gainFile.good()){
            ...
            if (! (first_header_entry == expected_first_column_header)){
                sprintf(errorMessage, "The first word of the header line is '%s'. It was expected to be '%s'. Please double check file format!!", 
...
void Detector::SetupInstalledStations(Settings *settings1=nullptr) {
    ...
    if (InstalledStations.size() > 0) { // Testbed
        ...
        // Make string 0        
        Antennas.push_back(4); 
        Antennas.push_back(1);
        InstalledStations[0].VHChannel.push_back(Antennas);
        Antennas.clear();
        
        // Make string 1
        Antennas.push_back(2); 
        Antennas.push_back(7);
        InstalledStations[0].VHChannel.push_back(Antennas); 
        Antennas.clear();
        
        // Make string 2
        Antennas.push_back(6); 
        Antennas.push_back(3);
        InstalledStations[0].VHChannel.push_back(Antennas); 
        Antennas.clear();
        
        // Make string 3
        Antennas.push_back(5); 
        Antennas.push_back(8);
        InstalledStations[0].VHChannel.push_back(Antennas); 
        Antennas.clear();
        
        // Make string 4
        Antennas.push_back(12); 
        Antennas.push_back(9);
        InstalledStations[0].VHChannel.push_back(Antennas); 
        Antennas.clear();
        
        // Make string 5
        Antennas.push_back(14); 
        Antennas.push_back(13);
        InstalledStations[0].VHChannel.push_back(Antennas); 
        Antennas.clear();
...
int Detector::GetChannelfromStringAntenna ( int stationNum, int stringnum, int antennanum, Settings *settings1) {
    ...
    return -1; // if we get to here something broke
}
...
void Detector::UseAntennaInfo(int stationNum, Settings *settings1){
    ...
    for ( int chan = 1; chan < InstalledStations[stationNum].nChannels+1; chan++){
        ...
        if (araGeom->getStationInfo(stationNum, settings1->DETECTOR_YEAR)->fAntInfo[chan-1].polType != AraAntPol::kSurface){
            ...
            stations[stationNum].strings[stringNum].antennas[antennaNum].type = int(araGeom->getStationInfo(stationNum, settings1->DETECTOR_YEAR)->fAntInfo[chan-1].polType);  
